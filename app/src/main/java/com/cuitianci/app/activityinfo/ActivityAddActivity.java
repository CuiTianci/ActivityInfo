...
public class ActivityAddActivity extends BaseActivity implements View.OnClickListener {
    private Activity receivedActivity;
    private String addOrEdit;
    private EditText name, summary, description, startTime, endTime, time, place, organization, mobile;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        summary = (EditText) findViewById(R.id.add_activity_summnary);
        description = (EditText) findViewById(R.id.add_activity_description);
        startTime = (EditText) findViewById(R.id.add_show_start_time);
        endTime = (EditText) findViewById(R.id.add_show_end_time);
        time = (EditText) findViewById(R.id.add_activity_time);
        place = (EditText) findViewById(R.id.add_activity_place);
        organization = (EditText) findViewById(R.id.add_activity_organization);
        mobile = (EditText) findViewById(R.id.add_activity_mobile);
        department = (Spinner) findViewById(R.id.add_activity_department_spinner);
        grade = (Spinner) findViewById(R.id.add_activity_grade_spinner);
        button = (Button) findViewById(R.id.add_activity_button);
        //添加点击监听
        ...
        if(addOrEdit.equals("edit")){
            ...
            time.setText(receivedActivity.getTime());
            place.setText(receivedActivity.getPlace());
            organization.setText(receivedActivity.getPlace());
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    break;
                case 1:
                    Toast.makeText(ActivityAddActivity.this,"保存成功",Toast.LENGTH_SHORT).show();
                    finish();
                    break;
                default:
                    break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.add_show_start_time:
                showTimePicker();
                whichTime = "1";
                break;
            case R.id.add_show_end_time:
                showTimePicker();
                whichTime = "2";
                break;

            case R.id.add_activity_time:
                showTimePicker();
                whichTime = "3";
                break;
    ...
    private void showTimePicker() {
        new DatePickerDialog(ActivityAddActivity.this, onDateSetListener, mYear, mMonth, mDay).show();
    }
    ...
    private void initSpinners() {
        ...
        //适配器
        department_arr_adapter = new ArrayAdapter<String>(ActivityAddActivity.this, android.R.layout.simple_spinner_item, department_data_list);
        //设置样式
        ...
        //适配器
        grade_arr_adapter = new ArrayAdapter<String>(ActivityAddActivity.this, android.R.layout.simple_spinner_item, grade_data_list);
        //设置样式
    ...
    private void saveData() {
        if (TextUtils.isEmpty(name.getText().toString()) || TextUtils.isEmpty(summary.getText().toString()) || TextUtils.isEmpty(description.getText().toString()) || TextUtils.isEmpty(startTime.getText().toString()) ||
                TextUtils.isEmpty(endTime.getText().toString()) || TextUtils.isEmpty(organization.getText().toString()) || TextUtils.isEmpty(mobile.getText().toString()) || TextUtils.isEmpty(time.getText().toString()) || TextUtils.isEmpty(place.getText().toString()) ){
                Toast.makeText(ActivityAddActivity.this,"数据不完整，保存失败",Toast.LENGTH_SHORT).show();
            return ;
        }
        String httpUrl;
        ...
        formBody.add("time", time.getText().toString());
        formBody.add("place", place.getText().toString());
